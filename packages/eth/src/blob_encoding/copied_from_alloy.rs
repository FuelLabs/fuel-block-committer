...
use alloy::{
    consensus::{utils::WholeFe, BlobTransactionSidecar, Bytes48, EnvKzgSettings},
    eips::eip4844::{Blob, BYTES_PER_BLOB, FIELD_ELEMENTS_PER_BLOB},
};
...
impl PartialSidecar {
    ...
    fn fe_at_mut(&mut self, index: usize) -> &mut [u8] {
        let target_blob = index / FIELD_ELEMENTS_PER_BLOB as usize;
        let fe_offset_in_target_blob = index % FIELD_ELEMENTS_PER_BLOB as usize;
        &mut self.blobs[target_blob]
            [fe_offset_in_target_blob * 32..(fe_offset_in_target_blob + 1) * 32]
    }

    /// Get a mutable reference to the next unused field element.
    fn next_unused_fe_mut(&mut self) -> &mut [u8] {
        self.fe_at_mut(self.fe)
    }
...
impl<T: SidecarCoder> SidecarBuilder<T> {
    ...
    pub fn build_with_settings(
        ...
        for blob in self.inner.blobs.iter() {
            ...
            // SAFETY: same size
            unsafe {
                commitments.push(core::mem::transmute::<c_kzg::Bytes48, Bytes48>(
                    commitment.to_bytes(),
                ));
