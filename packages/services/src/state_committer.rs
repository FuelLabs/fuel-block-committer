...
mod tests {
    ...
    use setup::{mocks::l1::TxStatus, Blocks};
    ...
    async fn sends_fragments_in_order() -> Result<()> {
        //given
        let setup = setup::Setup::init().await;

        let fragment_tx_ids = [[0; 32], [1; 32]];

        let mut tx_listener = {
            let l1_mock = setup::mocks::l1::txs_finished([(fragment_tx_ids[0], TxStatus::Success)]);

            StateListener::new(l1_mock, setup.db(), 0, TestClock::default())
        };

        let mut importer = setup.importer_of_blocks(Blocks::WithHeights(0..1));
    ...
    async fn repeats_failed_fragments() -> Result<()> {
        ...
        let mut listener = {
            let l1_mock = setup::mocks::l1::txs_finished([(original_tx, TxStatus::Failure)]);
    ...
    async fn bundles_minimum_if_no_more_blocks_available() -> Result<()> {
        ...
        let secret_key = SecretKey::random(&mut rand::thread_rng());
        let blocks = (0..2)
            .map(|height| setup::mocks::fuel::generate_block(height, &secret_key))
            .collect_vec();

        let mut importer = setup.importer_of_blocks(Blocks::Blocks {
            blocks: blocks.clone(),
            secret_key,
        });
        importer.run().await?;
