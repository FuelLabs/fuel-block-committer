...
async fn something() {
    ...
    let current_block_height = 21402042;
    let starting_block_height = current_block_height - 24 * 3600 / 12;
    let data = client
    ...
    FeeAnalytics::new(local_client.clone()),
        services::state_committer::fee_optimization::Config {
            sma_activation_fee_treshold: 1000,
            short_term_sma_num_blocks: short_sma,
            long_term_sma_num_blocks: long_sma,
            comparison_strategy: services::state_committer::fee_optimization::ComparisonStrategy::StrictlyLessOrEqualByPercent(0.01)
        },
    );
