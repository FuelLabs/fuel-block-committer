...
use ports::{
    storage::{BundleFragment, DBCompressedBlock, SequentialFuelBlocks, Storage},
    types::{
...
impl Storage for DbWithProcess {
    delegate! {
        to self.db {
            ...
            async fn insert_blocks(&self, blocks: NonEmpty<DBCompressedBlock>) -> ports::storage::Result<()>;
